...
Feed.prototype.get = function get(req, res, next) {
    ...
    ws.once('text', function (msg) {
        ...
        self.websockets[uuid] = ws;

        ws.on('end', function () {
            if (self.websockets[uuid] !== undefined) {
                self.websockets[uuid].end();
                delete self.websockets[uuid];
            }
        });
                ...
                if (changeItem.changeKind.resourceObject.owner_uuid ===
                    ...
                    switch (changeKind.resource) {
                        case 'vm':
                            ...
                            if (typeof (self.websockets[uuid]) !== "undefined") {
                                var jsonMsg = JSON.stringify(wsMsg);
                                self.websockets[uuid].send(jsonMsg);
                            }
                            break;
                        default:
                            break;
